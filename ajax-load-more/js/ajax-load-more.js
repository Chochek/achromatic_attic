...
(function ($) {
    ...
    var page = 1,
        ...
        $data,
        $el = $('#news'),
        $content = $('#news > .aa-news-list'),
        $delay = 150,
        $scroll = true,
    	$prefix = 'aa-news-',
        $path = $content.attr('data-path'),
        $numOfPosts = $('#news > .num-of-posts').text().trim();
    ...
    AjaxLoadMore.init = function () {
        ...
        $el.append('<div class="load-more-btn-wrap col-xs-12"><a id="load-more" class="load-more">' + $button_text + '</a></div>');
        ...
        AjaxLoadMore.loadPosts = function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    //console.log($data);
                    var numOfLoadedPosts = $('.aa-news-list article').length;
                    if (($numOfPosts - numOfLoadedPosts) <= 3) {
                        $button.hide();
                        $button.parent().css('height', '108px')
                    }
                    if ($init) {
